...
// Setup interrupt
	pinMode(INTpin, INPUT_PULLUP);	//BH1792 has open drain output on interrupt pin
...
void BH1792::startMeasurement(void)
{
    ...
    I2C_hr.write(byte(MEAS_ST));          // Set measure start flag
    I2C_hr.endTransmission(true);         // stop transmitting & hang up
}
...
void BH1792::readFIFOdata(uint16_t *fifo_reg)
{
    if(ISRFlag_HR) {
        ISRFlag_HR=false;
        // TODO: Read 32 entries of FIFO
    }

}

void BH1792::readGreenData(void)
{
    uint8_t count = 4;
